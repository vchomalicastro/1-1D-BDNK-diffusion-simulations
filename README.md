...
- **`SA-PINN-ACTO - Data and figures/`**  
  ...
  - `test1a/`, `test1b/`, `test2a/`, `test2b/`, `test3a/`, `test3b/`  
    Each test folder contains `pinn_data.npz`, `pinn_data_adam_only.npz` (the best model obtained during Adam pretraining, before L-BFGS fine-tuning), `L2_results.txt`, `data.txt`, and plots:
...
The earlier version of this code, accompanying the arXiv preprint, is preserved on the `arxiv-version` branch of this repository.
